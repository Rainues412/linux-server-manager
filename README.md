...
> **当前版本：v1.2.0** | 更新日期：2026-07-10 | [更新日志](#更新日志)
...
- **AWR 快照不足时支持重新输入**：查询时间范围内无快照或快照少于 2 个时，提示是否重新输入时间范围，无需返回主菜单
- **AWR 拉取支持中途退出**：在时间输入和快照 ID 选择环节，输入 `q` 可取消并返回主菜单（不断开连接）
- **报告文件名加时间戳**：AWR 报告文件名增加时间戳后缀 (`YYYYMMDD_HHMMSS`)，避免覆盖历史报告

### v1.1.0 (2026-07-10)

- **修复 AWR 快照查询兼容性**：优化 Oracle AWR 快照查询和报告生成的兼容性处理

### v1.0.0 (2026-07-09)

- **首次发布**：Linux 远程服务器管理工具
- SSH 连接管理（密码/密钥认证）
- 系统基本信息快速采集
- 完整负载诊断（6 大维度检测）
- 交互式远程 Shell
